 ...
 */
public class MiIPartInspector extends MiEventHandler
	{
	private		MiPartInspector	inspector;

	public				MiIPartInspector()
		{
		addEventToCommandTranslation(
			Mi_EXECUTE_COMMAND_NAME, Mi_KEY_PRESS_EVENT, 'i', Mi_CONTROL_KEY_HELD_DOWN);
		}
	...
	protected	int		processCommand()
		...
		if (inspector == null)
			{
			inspector = new MiPartInspector();
			}
		MiPart obj = event.getTargetList().elementAt(0);
		if (obj == null)
			inspector.setTargetOfCommand(event.editor);
		else
			inspector.setTargetOfCommand(obj);
		inspector.processCommand(null);

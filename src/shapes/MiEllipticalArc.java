 ...
 */
public class MiEllipticalArc extends MiPart
	{
	...
	private		MiDistance	hRadius 	= 1;
	private		MiDistance	vRadius 	= 1;
	private		double		startAngle 	= 0.0;
	private		double		sweptAngle	= 90.0;



	public				MiEllipticalArc()
		{
		}

	public				MiEllipticalArc(MiCoord x, MiCoord y, 
						MiDistance hRadius, MiDistance vRadius,
	...
	public		void		render(MiRenderer renderer)
		...
		tmpBounds.xmin = center.x - hRadius;
		tmpBounds.ymin = center.y - vRadius;
		tmpBounds.xmax = center.x + hRadius;
		tmpBounds.ymax = center.y + vRadius;
		renderer.drawEllipticalArc(tmpBounds, startAngle, sweptAngle);
		}
					/**------------------------------------------------------
	...
	public		MiPropertyDescriptions	getPropertyDescriptions()
		...
		propertyDescriptions = new MiPropertyDescriptions("MiEllipticalArc");
	...
	public		void		copy(MiPart source)
		...
		MiEllipticalArc obj = (MiEllipticalArc )source;
	...
	protected	void		scaleParts(MiPoint center, MiScale scale)
		...
		hRadius = hRadius * scale.x;
		vRadius = vRadius * scale.y;
		}
					/**------------------------------------------------------
	...
	protected	void		reCalcBounds(MiBounds bounds)
		...
		bounds.union(
			center.x + hRadius * Math.cos(startAngle * Math.PI/180),
			center.y + vRadius * Math.sin(startAngle * Math.PI/180));

		bounds.union(
			center.x + hRadius * Math.cos((startAngle + sweptAngle) * Math.PI/180),
			center.y + vRadius * Math.sin((startAngle + sweptAngle) * Math.PI/180));
		...
		if ((startAngle <= 0) && (endAngle >= 0))
			bounds.union(center.x + hRadius + halfLineWidth, center.y);
		if ((startAngle <= 90) && (endAngle >= 90))
			bounds.union(center.x, center.y + vRadius + halfLineWidth);
	...
	public		String		toString()
		...
		tmp += "[center=" + center + ",hRadius=" + hRadius + ",vRadius = " + vRadius

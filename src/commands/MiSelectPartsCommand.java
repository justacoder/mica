 ...
 */
public class MiSelectPartsCommand extends MiCommandHandler implements MiiTransaction, MiiDisplayNames, MiiCommandNames
	{
	private		MiParts		parts			= new MiParts();
	private 	boolean		selectTheParts;


	public				MiSelectPartsCommand(MiEditor editor, MiParts parts, boolean selectTheParts)
		{
		setTargetOfCommand(editor);
		this.parts.append(parts);
		this.selectTheParts = selectTheParts;
		}
	public		MiEditor	getEditor()
		{
		return((MiEditor )getTargetOfCommand());
		}
	public		boolean		isSelectingTheParts()
		{
		return(selectTheParts);
		}
	public		void		processCommand(String arg)
		{
		processCommand(getEditor(), parts, selectTheParts);
		}
	public		void		processCommand(MiEditor editor, MiParts parts, boolean selectTheParts)
		{
		doit(editor, parts, selectTheParts);
		MiSystem.getTransactionManager().appendTransaction(
			new MiSelectPartsCommand(editor, parts, selectTheParts));
		}
	public		void		doit(MiEditor editor, MiParts parts, boolean selectTheParts)
		{
		MiiSelectionManager selectManager = editor.getSelectionManager();
		for (int i = 0 ; i < parts.size(); ++i)
			{
			if (selectTheParts)
				selectManager.selectObject(parts.elementAt(i));
			else
				selectManager.deSelectObject(parts.elementAt(i));
			}
		} 
					/**------------------------------------------------------
	...
	public		String		getName()
		{
		if (selectTheParts)
			return(Mi_SELECT_DISPLAY_NAME);
		else
			return(Mi_DESELECT_DISPLAY_NAME);
		}
					/**------------------------------------------------------
	...
	public		String		getCommand()
		{
		if (selectTheParts)
			return(Mi_SELECT_COMMAND_NAME);
		else
			return(Mi_DESELECT_COMMAND_NAME);
		}
					/**------------------------------------------------------
	...
	public		void		redo()
		{
		doit(getEditor(), parts, selectTheParts);
		} 
					/**------------------------------------------------------

 ...
 */
public class MiSnapPointManager extends MiManagedPointManager
	{
	public	static	MiSnapPointManager	noopSnapPointManager	= new MiNoopSnapPointManager();
	private	static	MiSnapPointRule		globalRule	= new MiSnapPointRule();
	private	static	MiPart			globalLook	= new MiRectangle(0,0,8,8);
	private static	boolean			globalVisiblity;
	private static	boolean			globallyHidden;


	public					MiSnapPointManager()
		{
		}
	...
	public static	void			setGlobalRule(MiSnapPointRule rule)
		{
		globalRule = rule;
		}
	public static	MiSnapPointRule	getGlobalRule()
		{
		return(globalRule);
		}
	...
	public		MiSnapPointRule	getRule(MiManagedPoint point)
		{
		if (point.getRule() != null)
			return((MiSnapPointRule )point.getRule());
		if (getLocalRule() != null)
			return((MiSnapPointRule )getLocalRule());
		return(globalRule);
		}

	public		MiManagedPointManager	getManager(MiPart part)
		{
		return(part.getSnapPointManager());
		}
	public		MiManagedPointManager copy()
		{
		MiSnapPointManager manager = new MiSnapPointManager();
		manager.copy(this);
		return(manager);
		}
	}
class MiNoopSnapPointManager extends MiSnapPointManager
	{
	public		MiManagedPoints		getManagedPoints()
		{
		return(new MiManagedPoints());
		}
	public		MiManagedPointManager copy()
		{
		MiSnapPointManager manager = new MiNoopSnapPointManager();
		manager.copy(this);
		return(manager);
		}
	}
